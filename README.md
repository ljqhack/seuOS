...
#####任务控制块
建立任务需要一个数据结构保存其相关信息，这个数据结构称为任务控制块。本工程的任务控制块仅仅包含了堆栈指针及用于延时挂起任务的变量。
```
typedef struct TCB{
...
```
定义好任务控制块后，本项目直接定义了包含32个任务控制块的数组，因此最大支持32个任务，每任务对应一个优先级。


#####就绪表及任务调度
任务调度的首要工作是确定就绪表中的最高优先级，这里我们采用了一种最笨的方法来实现，也就是遍历整个就绪表：
```
OSPrioHighRdy = 0;
...
```
实际上，该方法是存在问题的，因为对于一个实时内核，调度器完成一次调度所需要的时间以及这个时间是否确定是实时内核的一项重要指标。
如果采用上面的方法每调度一次遍历到最大优级的时间是不确定的，并不是最好的办法。最好的办法是使基于位图的优先级算法。具体实现可以参考uCOS II中的实现方法。

#####互斥访问临界区
本工程采用关中断的方法实现临界区互斥访问。
```
...
写到这里，我们的代码实际上存在一个非常大的bug，这个bug可以使系统很轻易的奔溃，是什么问题呢？

这个问题就是：代码中还没有处理**临界区**。代码临界区是指处理时不可分割的代码，一旦这部分代码开始执行，则不允许任何中断打入。为确保临界代码的执行不被中断，在进入临界前必须先关中断，而临界段代码完成后，要立即开中断。
如此一来，系统函数中涉及全局变量的地方，必须加上临界处理。
首先，进行如下宏定义：
```
#define DisInt()				__disable_irq()
#define EnInt()					__enable_irq()
```
定义好后，在seuos.c中相关函数中加入开关中断，以使临界资源保护起来。

### Step4.内存管理
我们平时编程所使用的标准c中malloc分配内存在嵌入式系统中并不安全，因为在内存非常有限的嵌入式系统中，多次调用malloc以后，内存慢慢会变得不连续，会导致很难分配到一块连续的内存，最终会使系统奔溃。
在RTOS中分配内存对时间也是有严格要求的，内存分配往往比通用操作系统要求苛刻的多。
针对这个问题，我们可以设计一个静态内存池管理的方法，也就是初始化一大块内存区域，然后划分为等大小的小内存块，把这样一个内存集合称为内存池，当需要使用的时候就从中拿一块，使用完后再放回内存池。
